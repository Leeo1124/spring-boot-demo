...
import com.leeo.sys.user.exception.UserException;
...
public class UserRealm extends AuthorizingRealm{
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        //到数据库查是否有此对象
        User user = this.userService.findByUsername(loginName);// 实际项目中，这里可以根据实际情况做缓存，如果不做，Shiro自己也是有时间间隔机制，2分钟内不会重复执行该方法
        if(user!=null){
            //权限信息对象info,用来存放查出的用户的所有的角色（role）及权限（permission）
            SimpleAuthorizationInfo info=new SimpleAuthorizationInfo();
            //用户的角色集合
            info.setRoles(this.userAuthService.findStringRoles(user));
            //用户的角色对应的所有权限，如果只使用角色定义访问权限，下面的四行可以不要
            info.addStringPermissions(this.userAuthService.findStringPermissions(user));
            // 或者按下面这样添加
...
//            logger.info("已为用户[mike]赋予了[admin]角色和[admin:manage]权限");
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(
        ...
        User user = null;
        try {
            user = this.userService.login(token.getUsername().trim(), new String(token.getPassword()));
