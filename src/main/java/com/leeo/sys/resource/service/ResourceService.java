package com.leeo.sys.resource.service;
...
import com.leeo.common.service.BaseTreeableService;
import com.leeo.sys.resource.entity.Menu;
import com.leeo.sys.resource.entity.Resource;
import com.leeo.sys.resource.repository.ResourceRepository;
...
public class ResourceService extends BaseTreeableService<Resource, Long> {
    ...
    public List<Resource> findMenu(){
    	return this.resourceRepository.findMenu();
    }
    ...
    public String findActualResourceIdentity(Resource resource) {
        ...
        StringBuilder s = new StringBuilder(hasResourceIdentity ? resource.getIdentity() : "");

        Resource parent = null != resource.getParentId() ? findOne(resource.getParentId()) : null;
        while(parent != null) {
        ...
        //取消通配符*，包含通配符后可以包含下级的任意资源/操作/实例，造成菜单只能一级一级控制，不能单独控制某一个菜单项，因此注销
        if(hasChildren) {
            s.append(":*");
        }
...
//        	menus.add(convertToMenu(resource));
//        }
//        
//        return compositeToMenus(menus);
    }
    
    public static List<Menu> compositeToMenus(List<Menu> list) {
